...
blur = cv2.GaussianBlur(img6,(3,3),0)           #removes disturbance, noise in image
...
for img in images:
    ...
    if ht2>wt2:     #resizing the image in proportion 28*28, case1  height>width
      ...
      if var%2==0:
        new2 = cv2.copyMakeBorder(new, 0, 0, int(var/2),int(var/2), cv2.BORDER_CONSTANT,value=white)
        ...
        new2 = cv2.copyMakeBorder(new, 0, 0, int(var/2),int(var/2)+1, cv2.BORDER_CONSTANT,value=white)
        new3 = cv2.copyMakeBorder(new2, 1, 1, 1,1, cv2.BORDER_CONSTANT,value=white)
        new4 = cv2.resize(new3, (28,28),interpolation = cv2.INTER_NEAREST)
      ...
      if var%2==0:
        new2 = cv2.copyMakeBorder(new, int(var/2),int(var/2),0,0, cv2.BORDER_CONSTANT,value=white)
        new3 = cv2.copyMakeBorder(new2, 1, 1, 1,1, cv2.BORDER_CONSTANT,value=white)
        new4 = cv2.resize(new3, (28,28),interpolation = cv2.INTER_NEAREST)

      else :
        new2 = cv2.copyMakeBorder(new, int(var/2),int(var/2)+1,0,0,cv2.BORDER_CONSTANT,value=white)
        new3 = cv2.copyMakeBorder(new2, 1, 1, 1,1, cv2.BORDER_CONSTANT,value=white)
        new4 = cv2.resize(new3, (28,28),interpolation = cv2.INTER_NEAREST)
